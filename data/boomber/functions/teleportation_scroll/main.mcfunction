...
execute as @e[type=item,nbt={Item:{Count:1b, tag:{boomber:{group:["teleportation_scroll"]}}}, OnGround:1b}] at @s if entity @e[type=item, nbt={Item:{id:"minecraft:redstone", Count:1b}, OnGround:1b}, distance=..0.5, limit=1] run function boomber:teleportation_scroll/event/teleportation_scroll/activate
